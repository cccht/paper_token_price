...
\begin{abstract}
大语言模型（large language model, LLM）推理服务在短期内受到固定图形处理器（graphics processing unit, GPU）容量约束，而请求到达量在一天内存在明显波动。峰值过载会降低服务质量（quality of service, QoS），但离峰容量即使闲置也仍然产生持有成本。本文构建一个可审计的仿真模型，用于检验分时定价能否在不扩充容量的情况下，将时间弹性推理需求从拥塞时段转移出去并保护 QoS。模型包含两个异质推理服务商、一个应用程序编程接口（application programming interface, API）中间商、时间刚性用户与时间弹性用户、服务商直连 API 选项以及外部退出选项。服务商价格由低维时间形状族表示，服务商竞争通过虚拟博弈和有限网格 regret 诊断近似。在拥塞设定下，统一定价使峰值利用率达到 $0.782$，最低 QoS 降至 $0.756$。动态粗网格和细网格快照分别将峰值利用率降至 $0.706$ 和 $0.666$，并将最低 QoS 提高至 $0.968$ 和 $0.990$。由于细网格纯策略快照仍具有较高 regret，本文在完整 225 点网格上增加双重 oracle 混合策略诊断；得到的 $5\times5$ 支撑集剖面最大 regret 为 $0.203$，期望峰值利用率为 $0.703$，最低 QoS 为 $0.970$。服务管理基线、结构消融、25 点固定策略相图以及五组受限局部重求解表明，QoS 增益比利润增益更容易复现。利润结果并不稳健：粗网格快照为 $+9.3\%$，细网格快照为 $-1.9\%$，低 regret 混合剖面约为 $-2.8\%$。因此，分时定价在本文中被解释为固定容量下的 QoS 保护机制，而不是可靠的利润提升机制。
\end{abstract}
...
本文位于三类研究的交叉位置。第一，在线推理服务系统研究已经讨论低延迟服务、批处理、内存管理、模型并行和 prefill/decode 分离 \cite{crankshaw2017clipper,gujarati2020clockwork,romero2021infaas,yu2022orca,li2023alpaserve,sheng2023flexgen,kwon2023pagedattention,agrawal2024sarathiserve,patel2024splitwise,zhong2024distserve,agrawal2024vidur}。这些研究解释了利用率升高时 QoS 为什么恶化，但通常没有把价格、用户退出和跨服务商竞争置于同一个市场层仿真模型中。第二，拥塞定价和收益管理长期研究如何在时变需求下用价格分配稀缺容量 \cite{vickrey1969congestion,arnott1990bottleneck,gallego1994dynamic,elmaghraby2003dynamic,bitran2003overview,talluri2004revenue,phillips2005pricing}，实时电价和需求响应则为削峰提供了经典类比 \cite{schweppe1988spot,borenstein2005long,allcott2011rethinking,siano2014demand}。第三，近期关于人工智能（artificial intelligence, AI）服务市场的研究开始分析推理成本、菜单定价、token 容量合约和 GPU 服务的 Stackelberg 型定价 \cite{demirer2025emerging,erdil2025inference,bergemann2025menu,guo2026prillm,yan2026stackelberg,cunningham2026token,liu2026locational}。本文目标更窄：提供一个可复现的仿真框架，将 QoS 拥塞、服务商异质性、API 中间商、服务商直连选项和用户退出共同表示出来。用户选择遵循标准离散选择和平台市场工具 \cite{mcfadden1974conditional,anderson1992discrete,train2009discrete,rochet2003platform}。

本文有三点贡献。第一，构建一个固定容量下的推理服务分时定价仿真模型，包含两个竞争性异质服务商、一个路由中间商、时间刚性与时间弹性用户以及外部选项。模型同时表示闲置容量持有成本、拥塞下 QoS 退化和用户需求的时段迁移。第二，使用低维价格形状族和虚拟博弈获得可复现的候选响应。朴素最优响应动态在拥塞区出现循环，而虚拟博弈产生时间平均策略；本文报告有限网格 regret，以限定均衡近似的强度。第三，本文通过求解器诊断、服务管理基线、结构消融、固定策略压力测试和受限局部重求解，将 QoS 结果与利润结果分开。这些诊断一致支持拥塞设定下有边界的 QoS 保护结论，但不支持稳健的利润提升结论。因此，本文将分时定价定位为一种潜在的服务质量管理工具，而不是一般性的收入增强机制。

\section{仿真模型}
\label{sec:model}

\subsection{市场结构与决策顺序}
...
\caption{市场结构。用户可以通过 API 中间商购买、直接连接服务商，或退出市场。中间商依据价格和 QoS 在两个服务商之间路由流量。}
\label{fig:market_schematic}
\end{figure}

\subsection{用户选择与外部选项}
...
\begin{equation}
  ...
  \label{eq:utility}
\end{equation}
其中，$b_k$ 表示渠道品牌价值或便利性，$a_t$ 表示时段偏好，$\pi^\theta_t$ 是类型 $\theta$ 的原生时段分布，$q_{k,t}$ 为渠道 QoS。迁移成本以价格等价单位表示，因此由同一个价格敏感度参数 $\alpha_\theta$ 缩放。两类用户仅在参数上不同：时间刚性用户价格敏感度较低、迁移成本较高，且原生时段分布集中于峰时；时间弹性用户价格敏感度较高、迁移成本较低，且原生时段分布更分散。
...
\end{equation}
主校准中 $\eta=1.2$。刚性/replay 响应在主校准中使用 $v_r=1.80$ 和 $\beta=5.0$。总需求是两类用户需求按人口权重 $(\pi_R,\pi_E)$ 加权求和。模型不是固定总需求模型。外部选项允许内部需求收缩，弹性市场规模项允许低价下需求扩张。因此，后文关于利润边界的讨论是一种机制分解和数值诊断，而不是建立在固定总需求上的定理。
...
\begin{equation}
  ...
  \label{eq:load}
\end{equation}
QoS 退化函数 $\qos(u)$ 在利用率超过阈值 $\bar u$ 后开始下降。由于 $G_B<G_A$，在相同进入负载下，小容量服务商更早进入拥塞区域。主实验使用平滑 sigmoid 型退化形状，阈值型、线性型和平方根型形状作为工件集中的稳健性检查保留。给定路由矩阵时，QoS 固定点通过阻尼迭代计算：
\begin{equation}
  \tilde q^{(n+1)}_{m,t}
...
\end{equation}
持有成本 $c_gG_m$ 作用于固定容量，而不是实际利用量。这使服务商有动机在低负载时段降价，以填充离峰容量。该设定区别于仅把容量作为固定上界、但忽略闲置资源成本的模型。
...
在给定报告网格、参数设置和求解器种子的条件下，仿真是确定性的。校核（verification）关注实现是否遵循第~\ref{sec:model}--\ref{sec:solver} 节中的记账、路由和固定点方程。验证（validation）的问题更窄：这些抽象是否足以可信地支持一个关于固定容量推理定价机制的研究。这些检查限定了本文主张，而不是认证一个生产推理服务或真实平台市场。
...
\subsection{非拥塞状态：需求发生迁移，但利润效应有限}
...
\subsection{拥塞状态：QoS 证据增强，而利润仍不稳定}

容量收紧后，统一定价将市场推入拥塞。表~\ref{tab:congested} 报告四个对象：统一定价基线、动态粗网格时间平均策略、动态细网格纯策略快照以及有限网格混合策略诊断。表~\ref{tab:evidence_boundary} 说明每个对象在论文主张结构中的使用方式。该比较将依赖求解器分辨率的结论，与在低可利用性有限网格混合剖面下仍成立的结论区分开来。

\begin{table}[H]
\centering
\caption{拥塞状态：时间平均、快照与混合策略诊断下统一定价与动态定价比较}
\label{tab:congested}
\small
\begin{tabular}{lcccc}
\toprule
指标 & 统一定价 & 动态粗网格 & 动态细网格快照 & 混合诊断 \\
\midrule
峰值利用率 & 0.782 & 0.706 & 0.666 & 0.703 \\
最低 QoS & 0.756 & 0.968 & 0.990 & 0.970 \\
系统利润 & 1783 & 1949 & 1749 & 1733 \\
利润变化 & --- & $+9.3\%$ & $-1.9\%$ & $-2.8\%$ \\
最大 regret & --- & 4.98 & 22.30 & 0.203 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{拥塞状态下的时段利用率与 QoS 曲线。动态策略在粗网格和细网格快照中都降低峰值利用率并提高服务商最低 QoS。利用率面板中的虚线标示 QoS 退化开始的利用率阈值。}
...
\caption{利润分布与单一快照收敛边界。粗网格达到低于 5 的 regret，而细网格纯策略快照在 40 轮后仍有 regret $22.3$。因此，细网格纯策略快照不用于形成关于利润或策略强度的稳健均衡结论。}
...
图~\ref{fig:qos_profiles}、图~\ref{fig:profit_regret} 和图~\ref{fig:mixed_oracle} 从时段、利润组成和 regret 角度展示同一组证据。QoS 改善方向在两个快照和混合剖面中都成立。统一定价使最低 QoS 降至 $0.756$，峰值利用率升至 $0.782$。动态定价快照将峰值利用率降至 $0.66$--$0.71$ 区间，并将最低 QoS 提升至 $0.97$--$0.99$ 区间。有限网格混合诊断的期望峰值利用率为 $0.703$，最低 QoS 为 $0.970$。因此，QoS 结果并非仅由高 regret 的细网格纯策略快照驱动，也受到低 regret 有限网格混合/平均剖面的支持。
...
在这一更强检查中，局部候选集包含静态、动态粗网格、动态细网格、仅离峰和仅峰时形状，每个场景运行四轮局部最优响应。表~\ref{tab:resolved_sensitivity} 在全部五行中都保持正 QoS 增益，增益范围为 $0.156$ 到 $0.280$。利润仍然混合。这是当前关于论文中心分离结论的最强证据：QoS 保护比利润提升更容易复现。

\section{讨论：QoS 改善与利润边界}
\label{sec:discussion}

拥塞状态下的中心发现是 QoS 显著改善，从 $0.756$ 提高到约 $0.97$ 或更高，而利润没有显示出稳定的正响应。由于利润效应在粗网格快照、细网格快照和低 regret 混合剖面之间符号变化，本文不把利润作为正向结果处理。相反，本文将其作为诊断，用于解释为什么 QoS 改善并不必然转化为服务商利润。

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{mechanism_diagnostics.pdf}
\caption{机制诊断。动态定价提高 QoS 调整后的服务量并降低退出概率，但平均支付价格同时下降。因此，该改善更接近覆盖率和用户体验改善，而不是稳定利润增长。}
\label{fig:mechanism}
\end{figure}

\subsection{剩余重分配而不是新增供给}
...
\subsection{为什么恢复的 QoS 损失无法可靠转化为利润}
...
在粗网格和细网格快照、有限网格混合策略诊断、服务管理基线、结构消融、局部压力测试、相图和受限局部重求解中，结果支持一个有边界但一致的 QoS 结论。在拥塞状态下，分时定价降低峰值负载，并将最低 QoS 从 $0.756$ 提高到约 $0.97$ 或更高。双重 oracle 诊断在完整 225 点细网格上将最大 regret 降至 $0.203$，额外的 SMPT 诊断表明 QoS 增益比利润增益更容易复现。利润结果并不稳健：利润效应的符号随求解对象和网格分辨率变化。因此，本文不声称分时定价能够可靠提升利润。相反，结果指向一种剩余重分配机制：在固定容量、竞争和用户退出条件下，价格变化可以恢复拥塞损失并降低退出，但相关收益可能被需求外流、更薄边际和剩余转移抵消。对于运营者和监管者，分时定价更应被视为服务质量管理工具，而不是可靠的收入增长工具。未来工作应纳入真实轨迹校准，在更大或连续策略空间中验证低可利用性，并将模型扩展到更多服务商和中间商。

\section*{可复现性、数据可用性与工件边界}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \path{experiments/run_peak_shaving_smpt_experiments.py}: SMPT 基线、消融、相图、受限局部重求解和固定点残差诊断；
...
\section*{生成式 AI 与 AI 辅助技术声明}

在稿件准备过程中，AI 辅助工具被用于语言润色、一致性检查和内部审稿式诊断。作者仍对仿真模型、实验工件、数值结果、解释和最终文本负责。
