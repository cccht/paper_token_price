...
\caption{固定容量推理服务仿真与有限网格诊断框架。图 (a) 展示时间刚性和时间弹性用户在 API 中间商、服务商直连渠道与外部选项之间进行选择；中间商选择零售价，并向两个容量异质服务商路由需求，服务商负载进一步决定利用率和 QoS。图 (b) 依次评估服务商价格形状候选、中间商响应、渠道--时段选择、联合路由--QoS 固定点、服务商收益矩阵、虚拟博弈与双重 oracle。实线表示请求或路由流量，虚线表示价格和 QoS 信号。输出仅对应有限网格 regret、QoS 保护和利润边界，不表示连续策略空间中的 Nash 证明。图标来自 \href{https://streamlinehq.com}{Streamline Ultimate Color}，并依据 \href{https://creativecommons.org/licenses/by/4.0/}{CC BY 4.0} 使用。}
...
图~\ref{fig:market_schematic} 的可编辑源文件为 \path{figure_sources/peak_shaving_framework_2026-07-11.drawio}，确定性生成器为 \path{figure_sources/build_peak_shaving_framework_drawio.py}。论文采用 PDF 1.3 兼容导出；无损 PNG 和 SVG 导出与源文件一并保留。本地固定版本的 Streamline 图标、源文件映射、署名文本和 CC BY 4.0 许可文件存储在 \path{figure_sources/iot_icon_candidates/streamline_ultimate_color/} 下。
...
在稿件准备过程中，AI 辅助工具被用于语言润色、一致性检查、内部审稿式诊断，以及可编辑矢量图的版式和一致性检查；未使用生成式图像模型创建或修改该矢量图稿。图~\ref{fig:market_schematic} 根据本文模型公式和仿真流程构建，使用了获得许可的 Streamline 图标，并由作者审阅确认。作者仍对仿真模型、实验工件、数值结果、解释、文本和图稿负责。
