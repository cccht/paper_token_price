...
\title{\textbf{Simulation-based analysis of time-of-use pricing for fixed-capacity inference services}}
...
\begin{abstract}
Large language model (LLM) inference services operate under fixed short-run graphics processing unit (GPU) capacity, while intraday demand variation can create off-peak idleness and peak quality-of-service (QoS) degradation. This study develops a stylized simulation model with synthetic calibration to examine whether time-of-use pricing can shift time-flexible demand away from congested periods and protect QoS without capacity expansion. The market includes two heterogeneous inference providers, one application programming interface (API) intermediary, time-rigid and time-flexible users, direct access, and an outside option. Provider prices follow a low-dimensional temporal shape, and competition is represented as a finite candidate game evaluated with fictitious play and regret diagnostics. Fixed-point residuals and solver traces verify the implementation; public price scales and controlled vLLM overload measurements provide limited validation anchors. In the congested uniform-pricing baseline, peak utilization is $0.782$ and minimum QoS is $0.756$. Dynamic coarse-grid and fine-grid snapshots reduce peak utilization to $0.706$ and $0.666$, respectively, and raise minimum QoS to $0.968$ and $0.990$. On the full 225-point grid, the main mixed-strategy diagnostic yields a $5\times5$ support profile with maximum regret $0.203$, expected peak utilization $0.703$, and minimum QoS $0.970$. Additional service-management baselines and stress tests preserve the positive QoS direction. Profit remains solver-dependent: $+9.3\%$ for the coarse snapshot, $-1.9\%$ for the fine snapshot, and approximately $-2.8\%$ for the low-regret mixed profile. These results support time-of-use pricing as a bounded QoS-protection mechanism, but not as a production forecast or a reliable source of higher profit.
\end{abstract}
...
The main gap is not the absence of another pricing heuristic. It is the absence of a reproducible simulation model that places congestion-sensitive QoS, provider competition, intermediary routing, direct-provider choice, and market exit in the same fixed-capacity setting. The closest modeling template comes from electricity demand-response studies, where time-varying prices are tested through explicit demand, utility, load, and welfare calculations. The present paper adapts that style of modeling and numerical reporting to inference services while keeping the engineering mechanism distinct.

The study makes three contributions. First, it formulates a market-level simulation framework for fixed-capacity inference services in which provider prices, intermediary routing, user exit, and congestion-sensitive QoS are evaluated in one accounting system. Second, it states the equilibrium evidence as a finite-grid diagnostic rather than as a continuous-space solution: candidate pricing shapes are evaluated through a QoS fixed point, fictitious play provides averaged responses, and unilateral-deviation regret reports the remaining exploitability. Third, it identifies a separation between service-quality and profit effects. In the congested calibration, time-of-use pricing consistently improves the QoS direction across the reported diagnostics, but the sign of the profit effect changes across solver objects and perturbations. The paper therefore positions time-of-use pricing as a bounded service-quality management mechanism rather than a general revenue-enhancement result.

\section{Related work}
\label{sec:related}

\subsection{Inference serving and market-level analysis}

Most inference-serving research studies decisions inside a computing system. Prior work has examined low-latency serving, batching, memory management, model parallelism, prefill/decode separation, and serving simulators \cite{crankshaw2017clipper,gujarati2020clockwork,romero2021infaas,yu2022orca,li2023alpaserve,sheng2023flexgen,kwon2023pagedattention,agrawal2024sarathiserve,patel2024splitwise,zhong2024distserve,agrawal2024vidur}. This literature establishes the engineering basis for the present model: latency and completion quality can deteriorate sharply when incoming load approaches local capacity. These studies usually treat workloads as inputs, and scheduling begins after requests arrive. Their main scope does not include prices, user switching, intermediary routing, or market exit.

Market-oriented studies of AI services address a different part of the problem. Recent work examines inference costs, menu pricing, token-capacity contracts, and Stackelberg-style GPU-service pricing \cite{demirer2025emerging,erdil2025inference,bergemann2025menu,guo2026prillm,yan2026stackelberg,cunningham2026token,liu2026locational}. These studies show that serving cost and capacity constraints should enter pricing decisions. Less attention has been given to the feedback that starts with price, changes channel and time choice, alters provider load, and then changes QoS. The present study examines this market-level feedback. It does not propose a new serving scheduler or a general contract design.

\subsection{Economic and empirical foundations of electricity pricing}

Electricity pricing is the closest established field for studying demand shifts under short-run capacity scarcity. Schweppe et al. developed spot pricing around the time- and location-dependent cost of supplying electricity \cite{schweppe1988spot}. Borenstein later examined how real-time retail prices can improve long-run efficiency when wholesale costs vary across hours \cite{borenstein2005long}. The central idea is that a flat price hides scarcity, whereas a time-varying price gives users a reason to reduce or reschedule peak demand. GPU capacity has a similar short-run scarcity, although the physical source and consequence of congestion are different.

Empirical electricity studies also show why a price signal cannot be treated as a guaranteed load shift. In a survey of 15 experiments, Faruqui and Sergici report peak reductions of 3--6\% under time-of-use tariffs and 13--20\% under critical-peak pricing. Reductions reached 27--44\% when critical-peak tariffs were combined with enabling technologies \cite{faruqui2010household}. Allcott's field study found that households reduced peak use under hourly prices but did not increase average off-peak consumption \cite{allcott2011rethinking}. These results separate peak conservation from movement into a low-price period. The same distinction matters for inference demand because a user may defer a request, switch provider, buy through another channel, or leave the market.

Demand-response surveys place these findings in a broader operating context \cite{albadi2008summary,siano2014demand}. Dynamic pricing depends on metering, communication, user flexibility, and the tariff presented to customers. Response also varies across user groups and technologies. The present model reflects this evidence by distinguishing time-rigid and time-flexible users and by retaining an outside option. It does not treat the synthetic response parameters as empirical estimates of inference users.

\subsection{Game-theoretic electricity pricing and solution methods}

Utility-based and game-theoretic electricity models make the price-response link explicit. Samadi et al. connect real-time prices with user utility maximization, while Yang et al. formulate time-of-use pricing as a strategic game \cite{samadi2010optimal,yang2013game}. Their common contribution is methodological: user demand and the seller's pricing decision are solved within the same model. This structure provides the main mathematical precedent for the utility, demand, and provider-profit functions used in this paper.

Leader-follower models add a clear sequence of decisions. Yu and Hong model one utility and multiple users as a Stackelberg game, with prices coordinating supply-demand balance and aggregate-load smoothing \cite{yu2016supply}. Srinivasan et al. compare real-time, time-of-use, and day-night tariffs using load and price data from Singapore \cite{srinivasan2017game}. Their study also reports that the required price-elasticity values were taken or adapted from United States data. This example shows that practical load data do not remove the need to justify behavioral calibration. The same issue appears here: the current artifacts do not contain reliable price-elasticity estimates for inference users.

These electricity games provide a strong method base, but their equilibrium statements remain tied to their own market structure. The representative models commonly use one utility or retailer and treat physical supply-demand balance as the main system constraint. The present market has two competing providers, an intermediary, direct purchase, and exit. Every provider strategy must also be evaluated through a routing and QoS fixed point. This embedded response can make provider payoffs non-smooth near congestion.

For this reason, the paper evaluates linear temporal price shapes at a finite set of parameter values and reports unilateral-deviation regret. The grid discretizes the parameters of the linear price function; it does not replace that function with unrelated period-by-period prices. The resulting certificate describes exploitability within the finite candidate game. It is not presented as a proof of Nash equilibrium in the unrestricted continuous strategy space.

\subsection{Congestion pricing, channel choice, and the remaining gap}

Congestion-pricing and revenue-management studies explain how prices allocate scarce capacity when demand varies over time \cite{vickrey1969congestion,arnott1990bottleneck,gallego1994dynamic,elmaghraby2003dynamic,bitran2003overview,talluri2004revenue,phillips2005pricing}. Discrete-choice models provide a basis for price-sensitive user decisions, while platform-market models explain why an intermediary can affect transactions between sellers and buyers \cite{mcfadden1974conditional,anderson1992discrete,train2009discrete,rochet2003platform}. Each stream covers part of the present setting. No single stream supplies the full combination of provider competition, routing, direct purchase, exit, and congestion-sensitive service quality.

\begin{table}[H]
\centering
\caption{Position of the present study relative to electricity-pricing and adjacent research.}
\label{tab:literature_positioning}
\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.29\textwidth}@{}}
\toprule
Research stream & Main contribution & Element retained here & Limit when transferred to inference services \\
\midrule
Spot and real-time electricity pricing \cite{schweppe1988spot,borenstein2005long}
& Relates time-varying prices to scarcity and system cost.
& A temporal price signal can reshape peak demand.
& Electricity cost and network constraints do not represent endogenous serving QoS. \\
Empirical demand response \cite{faruqui2010household,allcott2011rethinking}
& Shows that response varies by tariff, user, and enabling technology.
& Users differ in time flexibility, and peak reduction need not become off-peak demand.
& The evidence does not identify price elasticity for inference users. \\
Utility and game models \cite{samadi2010optimal,yang2013game,yu2016supply,srinivasan2017game}
& Solves pricing and user demand jointly and evaluates peak-load outcomes.
& Utilities, demand, profit, and strategic response are stated explicitly.
& The common seller-user hierarchy omits competing providers, API routing, and a QoS fixed point. \\
Congestion, revenue, and platform models \cite{vickrey1969congestion,talluri2004revenue,train2009discrete,rochet2003platform}
& Explains capacity allocation, price-sensitive choice, and intermediary roles.
& Capacity scarcity and channel choice enter the market model.
& These elements are rarely evaluated together with load-dependent inference quality. \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:literature_positioning} shows why the electricity analogy is useful but incomplete. In an electricity model, price changes demand that must be balanced against generation and network constraints. Here, price changes both time and channel choice. Those choices alter provider utilization, utilization changes QoS, and QoS feeds back into user choice and effective billed volume. This loop can improve QoS without producing higher profit.

The question addressed here is more specific than whether dynamic prices can shift demand. It asks whether an electricity-inspired time-of-use signal remains useful when provider competition, API routing, exit, and endogenous QoS are modeled together. It also asks how strongly that conclusion is supported when equilibrium evidence is limited to a finite strategy grid. The study tests a mature demand-response idea in a new service setting. It does not import the physical electricity model or claim production calibration.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{peak_shaving_framework_2026-07-11.pdf}
\caption{Framework of the fixed-capacity inference-service simulation and finite-grid diagnostic workflow. Panel (a) shows time-rigid and time-flexible users choosing among the API intermediary, direct provider access, and the outside option. The intermediary selects its retail price and routes demand to two capacity-heterogeneous providers; provider load determines utilization and QoS. Panel (b) evaluates provider price-shape candidates through the intermediary response, channel--period choice, the joint routing--QoS fixed point, the provider payoff matrix, fictitious play, and the double-oracle check. Solid arrows denote requests or routed traffic; dashed arrows denote price and QoS signals. The reported outputs are finite-grid regret, QoS protection, and the profit boundary; the diagram does not imply a continuous-space Nash proof. Icons are from \href{https://streamlinehq.com}{Streamline Ultimate Color} and are used under \href{https://creativecommons.org/licenses/by/4.0/}{CC BY 4.0}.}
...
\begin{equation}
  ...
  \label{eq:utility}
\end{equation}
Here $b_k$ denotes channel brand value or convenience, $a_t$ denotes period preference, $\pi^\theta_t$ is the native-period distribution for type $\theta$, and $q_{k,t}$ is channel QoS. Switching cost is represented in price-equivalent units and is scaled by the same price-sensitivity parameter $\alpha_\theta$. The choice set is the set of channel--period alternatives plus the outside option. The native-period distribution does not impose a hard sequential arrival process; instead, it enters Eq.~(\ref{eq:utility}) as the cost of serving or replaying demand away from a type's preferred period. Thus, ``time-rigid'' means high migration cost and peak-concentrated preference, not an absolute inability to move. Time-flexible users have higher price sensitivity, lower switching cost, and a more dispersed native-period distribution.
...
\end{equation}
with $\eta=1.2$ in the main calibration. The rigid/replay response uses $v_r=1.80$ and $\beta=5.0$ in the main calibration. Equation~(\ref{eq:demand}) should be read as a compact simulation device rather than a structural estimate of user arrivals. The logit share selects where the demand is served, the native-period term penalizes temporal displacement, and the replay term adds price-sensitive residual demand tied to the period load profile. Total demand is the population-weighted sum over the two types, with weights $(\pi_R,\pi_E)$. The model is not a fixed-total-demand model. The outside option allows internal demand to contract, and the elastic market-size term allows demand to expand at lower prices. The later discussion of the profit boundary is a numerical decomposition, not a theorem based on fixed total demand.
...
\begin{equation}
  ...
  \label{eq:load}
\end{equation}
The main experiments use a smooth sigmoid QoS degradation function,
\begin{equation}
  \qos(u)=
...
\end{equation}
The parameter $\bar u$ is the center of the degradation curve: service quality remains close to one below it and decreases rapidly as utilization approaches and exceeds it. Since $G_B<G_A$, the smaller provider reaches lower QoS earlier under comparable incoming load. Threshold, linear, and square-root shapes are retained as robustness checks in the artifact set. For a fixed routing matrix, the QoS fixed point is computed by damped iteration:
\begin{equation}
  \tilde q^{(n+1)}_{m,t}
...
\end{equation}
The factor $q_{m,t}$ is interpreted as an effective completion or SLA-attainment factor. Multiplying billed volume by $q_{m,t}$ is therefore a conservative assumption that only successfully served demand is fully monetized. The degradation term $c_q(1-q_{m,t})L_{m,t}$ represents additional service-level cost, support cost, reputation loss, or contractual penalty associated with degraded service; it is not an estimate of a separately observed accounting charge. If failed API requests were partially billed in a production contract, the revenue term would be less conservative. The holding cost $c_gG_m$ is charged on fixed capacity rather than on realized utilization. It gives providers an incentive to fill off-peak capacity by lowering prices in low-load periods. This specification distinguishes the present model from formulations that treat capacity only as a fixed upper bound and ignore the cost of idle resources.
...
\end{equation}
The same interpretation applies to $q^I_t$: it is the intermediary channel's effective completion factor induced by routing across providers. System profit is
\begin{equation}
  \Pi^{\mathrm{sys}}=\Pi_I+\sum_{m\in\M}\Pi_m .
  \label{eq:system_profit}
\end{equation}

\subsection{Solution method and simulation diagnostics}
\label{sec:solver}

\subsubsection{Low-dimensional pricing-shape family}
...
\subsubsection{Candidate evaluation and finite-game regret certificate}
...
\subsubsection{Fictitious play and finite-grid regret}
...
\subsubsection{Algorithmic outline}
...
\subsection{Verification, validation, and credibility boundaries}
\label{sec:vv}

The simulation is deterministic conditional on the reported grids, parameter settings, and solver seeds. Verification asks whether the implementation follows the accounting, routing, and fixed-point equations defined above. Validation asks a narrower question: whether the abstractions are credible enough for a mechanism study of fixed-capacity inference pricing. These checks bound the claims; they do not certify a production inference service or a real platform market.
...
Two controlled vLLM single-GPU overload measurements are reused to anchor the QoS degradation shape. The QoS observation is defined as the service-level agreement (SLA) attainment rate for TTFT not exceeding 0.5 seconds. Figure~\ref{fig:vllm_qos_anchor} shows that both measurement profiles maintain an SLA attainment rate of one when normalized concurrency is no greater than one, and begin to decline after the healthy-concurrency point. For the 0.5B model, SLA attainment rates are $0.635$ and $0.500$ at normalized concurrency levels $1.71$ and $2.29$, respectively; the corresponding 95\% confidence interval half-widths are $0.007$ and $0.000$. For the 3B model, the rate is $0.667$ at normalized concurrency $1.71$ with zero cross-repeat variation in the stored aggregate. Fitting the threshold-type function $q(u)=\exp[-\kappa(u-\bar u)^2_+]$ gives $(\bar u,\kappa,\mathrm{RMSE})=(1.000,0.517,0.068)$ for the 0.5B profile and $(1.058,0.942,6.3\times10^{-10})$ for the 3B profile. These measurements provide a validation-style shape anchor for the QoS proxy; they do not replace real request traces, user elasticity estimates, or production SLA calibration. The raw aggregate tables and metadata are stored in \path{artifacts/vllm-study/20260531-190126/} and \path{artifacts/vllm-study-qwen25-3b/20260531-214710/}.
...
After capacity is tightened, uniform pricing pushes the market into congestion. Table~\ref{tab:congested} places the uniform-pricing baseline next to the principal finite-grid mixed diagnostic. The coarse-grid time-averaged strategy and the fine-grid pure snapshot are retained in the same table only as solver diagnostics, not as equivalent equilibrium objects. Table~\ref{tab:evidence_boundary} then states how each object is used in the paper's claim structure.

\begin{table}[H]
\centering
\caption{Congested regime: main comparison and solver-diagnostic snapshots}
\label{tab:congested}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
Metric & Uniform baseline & Principal mixed diagnostic & Coarse-grid diagnostic & Fine-grid diagnostic \\
\midrule
Peak utilization & 0.782 & 0.703 & 0.706 & 0.666 \\
Minimum QoS & 0.756 & 0.970 & 0.968 & 0.990 \\
System profit & 1783 & 1733 & 1949 & 1749 \\
Profit change & --- & $-2.8\%$ & $+9.3\%$ & $-1.9\%$ \\
Maximum regret & --- & 0.203 & 4.98 & 22.30 \\
Regret / system profit & --- & $0.012\%$ & $0.26\%$ & $1.28\%$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
Figures~\ref{fig:qos_profiles}, \ref{fig:profit_regret}, and \ref{fig:mixed_oracle} visualize the same evidence from the period, profit-component, and regret perspectives. The central comparison is between the congested uniform baseline and the low-regret finite-grid mixed profile. Uniform pricing lowers the minimum QoS to $0.756$ and raises peak utilization to $0.782$, whereas the mixed profile has expected peak utilization $0.703$ and minimum QoS $0.970$. The coarse-grid and fine-grid snapshots show the same QoS direction, but they are used as solver-resolution diagnostics: the coarse snapshot is low-resolution, and the fine snapshot remains high-regret. The QoS conclusion is therefore stated for the finite-grid mixed/averaged profile and checked for directional consistency in the snapshots.
...
\subsection{Service-management baselines and structural checks}
...
\begin{table}[H]
\centering
\caption{Service-management baseline diagnostics in the congested setting}
...
For this stronger check, the local candidate set contains the static, dynamic coarse, dynamic fine, off-peak-only, and peak-only shapes, and each scenario runs four local best-response rounds. Table~\ref{tab:resolved_sensitivity} keeps positive QoS gain in all five rows, with gains between $0.156$ and $0.280$. Profit remains mixed. Within the current design, this is the most restrictive robustness check for the paper's central separation: the QoS direction persists under additional diagnostics, whereas the profit effect remains parameter- and solver-dependent.

\section{Conclusion and outlook}
\label{sec:discussion}
\label{sec:conclusion}

The central finding is simple: in the congested regime, time-of-use pricing protects QoS more reliably than it raises profit. Minimum QoS rises from $0.756$ to approximately $0.97$ or higher, whereas the profit effect changes sign across the coarse-grid snapshot, the fine-grid snapshot, and the low-regret mixed profile. Profit is therefore used as a diagnostic outcome, not as a positive result.
...
\subsubsection{Why recovered QoS losses do not reliably become profit}
...
% - Terminology standardized: simulation model, modeling, verification, validation anchor, QoS, API, GPU, LLM, TTFT, SLA.
% - Abstract compressed to the SMPT 250-word limit and keywords reduced to seven journal-appropriate terms.
% - Section titles and transitions refined to emphasize simulation-modeling contribution, validation anchors, finite-grid diagnostics, and bounded evidence.
