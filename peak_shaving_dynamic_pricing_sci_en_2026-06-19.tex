...
\title{\textbf{A Simulation-Based Study of Time-of-Use Pricing for Fixed-Capacity Artificial Intelligence Inference Services:\\
Quality-of-Service Protection and the Profit Boundary}}
...
\begin{abstract}
Artificial intelligence inference services with fixed graphics processing unit capacity often face pronounced within-day load variation. Peak periods can degrade latency and completion rates, whereas off-peak capacity remains costly even when it is underused. This paper develops an auditable simulation model for time-of-use pricing in an inference-service market with two capacity-heterogeneous providers, an application programming interface (API) intermediary, direct-provider channels, time-rigid and time-flexible users, quality-of-service (QoS) feedback, and outside-option exit. The model is implemented as a candidate-response simulation over a low-dimensional price-shape family. Fictitious play, fixed-point QoS routing, and finite-grid regret diagnostics are used to expose the computational limits of the solution procedure. In the congested synthetic setting, uniform pricing produces peak utilization $0.782$ and minimum QoS $0.756$. Dynamic coarse and fine candidate responses reduce peak utilization to $0.706$ and $0.666$ and increase minimum QoS to $0.968$ and $0.990$, respectively. A double-oracle mixed-strategy diagnostic on the full 225-point grid obtains maximum regret $0.203$, expected peak utilization $0.703$, and minimum QoS $0.970$. Local fixed-policy perturbations of capacity, price sensitivity, switching cost, and the QoS threshold preserve the QoS gains and peak-utilization reductions. Profit is less stable: the coarse response gives $+9.3\%$, the fine response $-1.9\%$, and the mixed profile about $-2.8\%$. These results support time-of-use pricing as a fixed-capacity QoS-protection mechanism, rather than as a reliable profit-improvement rule. Public API prices and controlled vLLM measurements are used only as price-scale and QoS-shape anchors; the economic calibration remains synthetic.
\end{abstract}

\noindent\textbf{Keywords:} simulation modelling; inference service; dynamic pricing; quality of service; peak shaving; finite-grid regret; sensitivity analysis

\section{Introduction}
\label{sec:intro}

Large-model inference has shifted from offline batch processing to continuously running online services. Chat, code generation, agent workflows, periodic evaluation, and scheduled batch tasks arrive at different times of day. Graphics processing unit (GPU) inference clusters therefore face load peaks that are difficult to remove through short-run capacity expansion \cite{crankshaw2017clipper,gujarati2020clockwork,yu2022orca,kwon2023pagedattention,agrawal2024sarathiserve}. When utilization approaches a local capacity threshold, time-to-first-token latency, queueing delay, and completion rates can deteriorate rapidly. The resulting loss of service quality also reduces the volume of requests that can be completed and billed.

This paper focuses on a fixed-capacity setting. GPU procurement and deployment involve lead times and sunk costs, and idle off-peak GPUs still incur holding costs. A natural operational response is to use prices rather than immediate capacity expansion: charge more in peak periods, charge less off peak, and shift time-flexible demand toward periods with spare serving capacity. The idea is close in spirit to real-time electricity pricing and demand response \cite{schweppe1988spot,borenstein2005long,albadi2008summary,faruqui2010household,siano2014demand}. The engineering object, however, is different. Inference-service quality is governed by local GPU utilization, latency, and completion rates rather than generation constraints or power-flow equations.

Inference markets also differ from the single-platform monopoly assumed in many pricing models. The same open-source model family, such as GLM, can be deployed by more than one provider. When user-visible model capability is similar, competition depends on price, available capacity, and realized quality of service (QoS). Provider capacity can also be uneven. A smaller provider may enter congestion earlier when an intermediary routes traffic to it. These observations motivate a two-provider model with heterogeneous fixed capacities, one application programming interface (API) intermediary that routes traffic between providers, and users who may also buy directly from providers or leave the market.
...
The paper contributes a reproducible simulation workflow rather than a closed-form equilibrium theory. First, it formulates a fixed-capacity model of time-of-use inference pricing with two competing providers, one routing intermediary, time-rigid and time-flexible users, and an outside option. Second, it combines a low-dimensional pricing-shape family with fictitious play to obtain reproducible candidate responses, while reporting regret whenever the solver does not provide a strong equilibrium certificate. Third, it separates the QoS result from the profit result. The experiments support a finite-grid QoS-protection claim in the congested setting, but they do not support a reliable profit-improvement claim.

\section{Related Work and Positioning}
\label{sec:related}

The study is positioned between three literatures. Systems research on inference serving explains why batching, memory management, and scheduling affect latency and throughput \cite{crankshaw2017clipper,gujarati2020clockwork,romero2021infaas,yu2022orca,li2023alpaserve,sheng2023flexgen,kwon2023pagedattention,agrawal2024sarathiserve,patel2024splitwise,zhong2024distserve,agrawal2024vidur}. This work usually treats the request set as given and does not place prices, outside options, and cross-provider competition in the same market model. Congestion pricing and revenue management study how prices allocate scarce capacity under time-varying demand \cite{vickrey1969congestion,arnott1990bottleneck,gallego1994dynamic,elmaghraby2003dynamic,bitran2003overview,talluri2004revenue,phillips2005pricing}. Recent work on markets for artificial intelligence services has begun to discuss inference costs, menu pricing, token-capacity contracts, and Stackelberg-style pricing for GPU services \cite{demirer2025emerging,erdil2025inference,bergemann2025menu,guo2026prillm,yan2026stackelberg,cunningham2026token,liu2026locational}. The user-choice component follows standard discrete-choice and platform-market tools \cite{mcfadden1974conditional,anderson1992discrete,train2009discrete,rochet2003platform}.

The gap addressed here is methodological and application-specific. The paper does not propose a new serving scheduler, a new revenue-management theorem, or a production demand forecast. Its contribution is a transparent simulation model that places QoS congestion, provider heterogeneity, intermediary routing, direct-provider alternatives, and market exit in the same fixed-capacity experiment, and then distinguishes evidence that is numerically well supported from evidence that remains bounded by the solver and calibration design.

\section{Conceptual Simulation Model}
\label{sec:model}

The conceptual model represents one inference-service day as interacting price, routing, demand, load, QoS, and profit components. It is not intended to be a digital twin of a specific provider. The modelling objective is to retain enough structure to trace how a price-shape change moves demand across time and channels, while keeping the implementation small enough for artifact-level verification.
...
\begin{equation}
  ...
  \label{eq:utility}
\end{equation}
Here $b_k$ denotes channel convenience, $a_t$ denotes period preference, $\pi^\theta_t$ is the native-period distribution, and $q_{k,t}$ is channel QoS. Switching cost is expressed in price-equivalent units and scaled by the same price-sensitivity parameter $\alpha_\theta$. Time-rigid users have lower price sensitivity, higher switching cost, and a more peaked native-period distribution.
...
\begin{equation}
  ...
  \label{eq:load}
\end{equation}
The QoS function begins to decline once utilization exceeds a threshold $\bar u$. Because $G_B<G_A$, the smaller provider reaches the congestion region earlier under the same incoming load. The main experiments use a smooth sigmoid-type degradation function. Threshold, linear, and square-root variants are used as robustness checks in the artifact set.

\subsection{Profit}

Provider profit is defined as wholesale revenue plus direct retail revenue, minus fixed capacity holding cost and QoS degradation cost:
\begin{equation}
...
With baseline capacity, uniform pricing reaches only $0.213$ peak utilization and QoS remains one. Dynamic pricing still shifts demand, but the regime leaves little room for QoS improvement.
...
The demand-shift result is consistent with the model design. The time-flexible group shifts by about 2.3 times as much as the time-rigid group. Profit, however, is neutral to mildly negative in this regime. Across the elastic-population scan, gains are $0\%,-0.32\%,-4.51\%,-3.76\%,-3.00\%$. In this model, dynamic pricing protects QoS only when congestion is present.

\subsection{Congested regime}

Tightening capacity creates a congested case under uniform pricing. Table~\ref{tab:congested} reports uniform pricing, the dynamic coarse-grid time-averaged strategy, the dynamic fine-grid pure snapshot, and the finite-grid mixed diagnostic.
...
The profit result is different. System profit changes from $+9.3\%$ on the coarse grid to $-1.9\%$ for the fine snapshot and about $-2.8\%$ for the mixed profile. Profit improvement is not treated as a positive finding. The supported reading is narrower: time-of-use pricing protects QoS in the congested fixed-capacity setting, but the current evidence does not show a reliable profit gain.

The provider-heterogeneity pattern is also unstable. A smaller provider might be expected to use stronger dynamic pricing, but the direction changes with grid resolution. The coarse grid gives $\delta_A=0.383$ and $\delta_B=0.023$; the fine grid gives $\delta_A=0.059$ and $\delta_B=0.278$. The paper makes no claim about which provider becomes more dynamic.

\subsection{SMPT baseline diagnostics}

To address the risk that uniform pricing is too weak a baseline, four service-management diagnostics are added around the congested setting: off-peak discount only, peak surcharge only, dynamic coarse pricing with fixed equal routing, and an admission-control diagnostic. Table~\ref{tab:smpt_baselines} reports these baselines next to the static candidate response and the two reported dynamic snapshots.
...
The fixed-policy phase grid gives a broader local view: QoS gain and peak-utilization reduction are positive in all 25 capacity--elasticity grid points, with minimum QoS gain $0.094$. Profit is positive on this fixed-policy coarse-policy grid, ranging from $+2.0\%$ to $+15.5\%$. This does not overturn the profit-boundary result, because the coarse policy is kept fixed.

For a stronger check, a restricted local candidate set is re-solved at five perturbation points. The candidate set contains the static, dynamic coarse, dynamic fine, off-peak-only, and peak-only shapes, and each scenario runs four local best-response rounds. The re-solved rows keep positive QoS gain in all five cases, with gains between $0.156$ and $0.280$. Profit remains mixed: capacity tightening and a lower QoS threshold give $+12.5\%$ and $+11.0\%$, while higher price sensitivity and a larger flexible-user share give $-1.4\%$ and $-1.9\%$. This is the strongest current evidence for the paper's central separation: QoS protection is easier to reproduce than profit improvement.

\section{Mechanism and Profit-Boundary Discussion}
\label{sec:discussion}

The central empirical pattern is clear. QoS improves sharply in the congested setting, whereas profit does not improve reliably. Time-of-use pricing reallocates demand, but it does not create additional GPU supply.
...
Figure~\ref{fig:mechanism} reports the relevant accounting. Under uniform pricing, QoS-adjusted served volume is $2610$. Under the dynamic coarse and fine snapshots, it rises to $2865$ and $3043$. Rigid-user exit falls from $0.158$ to $0.145/0.119$, and flexible-user exit falls from $0.087$ to $0.075/0.044$. Dynamic pricing recovers some service volume that congestion would otherwise destroy.

That recovery does not automatically become profit. The average paid price falls from $0.761$ to $0.735/0.625$. Peak-period price increases can improve margins, but they also push price-sensitive demand toward the rival provider, the direct channel, or the outside option. Off-peak discounts use idle capacity, but they thin per-request margins. The mixed profile indicates that these opposing forces can leave providers with better QoS and no stable profit gain.

This is not a fixed-total-demand theorem. The model allows users to exit and allows market size to expand at lower prices. A more precise interpretation is surplus reallocation. Prices move demand across time and channels, and some of the recovered congestion loss appears as better completion and lower exit rather than as provider profit.

For operators, the implication is practical. Time-of-use pricing should not be presented as a dependable revenue lever under fixed capacity and competition. It is more defensible as a service-quality tool: a way to reduce peak overload, improve completion, and protect SLA-like metrics. For regulators or platform designers, the same point has the opposite implication. A higher peak price is not automatically evidence of extraction if it also reduces failed service. The welfare question depends jointly on prices, completion, exit, and user surplus.

\section{Limitations}
\label{sec:limitations}

The main limitation is calibration. Price sensitivity, switching cost, capacity cost, population mix, and outside-option utility are synthetic. The numerical percentages in the paper should be read as mechanism evidence, not production forecasts. Public API prices provide only a price-scale reference. The controlled vLLM scans provide only a QoS-shape reference under a single GPU, small models, short generations, and a small set of concurrency levels.

The solver evidence is also bounded. Provider strategies are restricted to a low-dimensional pricing-shape family. The fine-grid pure snapshot remains high-regret, and the double-oracle mixed profile lowers regret only on the finite 225-point candidate grid. The phase grid evaluates fixed policies under perturbations, and the re-solved sensitivity uses a restricted local candidate set rather than a full re-optimization at each point. These choices are appropriate for an auditable simulation study, but they should not be read as a continuous-space equilibrium proof or global uncertainty quantification.

The market structure is intentionally small. The model has two providers and one API intermediary. Many-provider competition, multiple intermediaries, long-term capacity contracts, differentiated model quality, and dynamic capacity procurement are outside the current scope. Deployment-oriented analysis would require request traces, context-length distributions, production completion curves, user elasticity estimates, and online experiments.

\section{Conclusion}
\label{sec:conclusion}

This paper examined whether time-of-use dynamic pricing can protect inference-service QoS when GPU serving capacity is fixed. In the congested synthetic setting, the answer is positive in a bounded finite-grid sense. Dynamic pricing lowers peak utilization and raises the minimum QoS, and a double-oracle mixed-strategy diagnostic reduces full-grid regret to $0.203$ on the 225-point candidate grid. Local fixed-policy perturbations preserve the same QoS direction.

The profit result is less favorable, but more informative than a simple positive result. Profit changes sign across solver objects, and the low-regret mixed profile is negative relative to uniform pricing. The evidence points to surplus reallocation: time-of-use prices recover some congestion loss and reduce exit, but competition, discounts, and outside options prevent that gain from reliably becoming provider profit. The paper's contribution is a reproducible way to observe this QoS-profit separation under fixed capacity, not a claim that dynamic pricing is a universal revenue-improvement tool.
