...
def draw_market_layer(draw: ImageDraw.ImageDraw) -> None:
    users = Box(65, 120, 385, 360, "", "#E9F5F1", "#00A087")
    inter = Box(590, 120, 620, 360, "", "#EAF2FA", "#3C5488")
    providers = Box(1350, 120, 385, 360, "", "#FDEDEA", "#E64B35")
...
def draw_simulation_layer(draw: ImageDraw.ImageDraw) -> None:
    rounded_box(draw, Box(95, 570, 1610, 160, "", "#EAF7F5", "#4DBBD5"), radius=24, width=3)
    ...
    for label, x in steps:
        small_box(draw, Box(x, 640, 205, 58, label, "#FFFFFF", "#4DBBD5"), 18, wrap=22)
    for x in [350, 600, 860, 1120, 1375]:
        arrow(draw, (x, 669), (x + 35, 669), "#3C5488", width=4)
    arrow(draw, (900, 480), (900, 570), "#8491B4", width=6)


def draw_diagnostic_layer(draw: ImageDraw.ImageDraw) -> None:
    rounded_box(draw, Box(190, 790, 1420, 118, "", "#F3F2F8", "#8491B4"), radius=24, width=3)
    ...
    for label, x, width in steps:
        small_box(draw, Box(x, 852, width, 42, label, "#FFFFFF", "#8491B4"), 17, wrap=25)
    for x in [495, 750, 1010]:
        arrow(draw, (x, 873), (x + 42, 873), "#8491B4", width=4)
    poly_arrow(draw, [(1495, 852), (1660, 852), (1660, 452)], "#8491B4", width=4, dashed=True)
    label_text(draw, (1585, 695), "diagnostic\nfeedback", 17, color="#666C8F")


def draw_legend(draw: ImageDraw.ImageDraw) -> None:
    rounded_box(draw, Box(120, 940, 1560, 54, "", "#FFFFFF", "#AEB6BF"), radius=16, width=2)
    items = [
        ("#00A087", "user demand / QoS status", False),
        ("#3C5488", "price, signal, or direct access", True),
        ("#E64B35", "routed load", False),
        ("#8491B4", "fixed-point / regret iteration", True),
...
def build_drawio() -> None:
    ...
    for box in [
        Box(65, 120, 385, 360, "Users\nchoose brokered, direct,\nor outside option", "#E9F5F1", "#00A087"),
        Box(590, 120, 620, 360, "API Intermediary\nsets retail price pₜ and routing rₘ,ₜ", "#EAF2FA", "#3C5488"),
        Box(1350, 120, 385, 360, "Inference Providers\npost prices under\nfixed GPU capacity", "#FDEDEA", "#E64B35"),
        Box(95, 570, 1610, 160, "Simulation fixed point", "#EAF7F5", "#4DBBD5"),
        Box(190, 790, 1420, 118, "Finite-grid equilibrium diagnostic", "#F3F2F8", "#8491B4"),
    ]:
        dx.add_rect(box, 24, True)
    for box in [
        Box(95, 240, 325, 68, "Time-rigid users R\npeak-native demand", "#F5FBF8", "#00A087"),
        Box(95, 330, 325, 68, "Time-flexible users E\nshiftable demand", "#F5FBF8", "#00A087"),
        Box(95, 420, 325, 42, "Outside option: no purchase", "#F7F7F7", "#777777"),
        Box(630, 260, 160, 82, "Retail\nprice pₜ", "#FFFFFF", "#4DBBD5"),
        Box(815, 260, 170, 82, "Routing\nrₘ,ₜ", "#FFFFFF", "#4DBBD5"),
        Box(1010, 260, 160, 82, "Channel\nQoS qᴵₜ", "#FFFFFF", "#4DBBD5"),
        Box(1380, 240, 325, 64, "Provider A\nlarge capacity G_A", "#FFF7F5", "#E64B35"),
        Box(1380, 324, 325, 64, "Provider B\nsmaller capacity G_B", "#FFF7F5", "#E64B35"),
        Box(1380, 408, 325, 44, "QoS qₘ,ₜ = qos(Lₘ,ₜ/Gₘ)", "#FFF7F5", "#E64B35"),
        Box(145, 640, 205, 58, "Prices\npₜ, wₘ,ₜ, pᴰₘ,ₜ", "#FFFFFF", "#4DBBD5"),
        Box(385, 640, 205, 58, "Choice shares\nsᶿₖ,ₜ and sᶿ₀", "#FFFFFF", "#4DBBD5"),
        Box(635, 640, 205, 58, "Demand and routing\nDᶿₖ,ₜ, rₘ,ₜ", "#FFFFFF", "#4DBBD5"),
        Box(895, 640, 205, 58, "Load and utilization\nLₘ,ₜ, uₘ,ₜ", "#FFFFFF", "#4DBBD5"),
        Box(1155, 640, 205, 58, "QoS and profit\nqₘ,ₜ, Π", "#FFFFFF", "#4DBBD5"),
        Box(1410, 640, 205, 58, "Regret inputs\nbest responses", "#FFFFFF", "#4DBBD5"),
        Box(245, 852, 250, 42, "Provider candidate grids\n64 coarse / 225 fine", "#FFFFFF", "#8491B4"),
        Box(540, 852, 210, 42, "Intermediary response\ngrid Y", "#FFFFFF", "#8491B4"),
        Box(790, 852, 220, 42, "Regret scan\nρ = max deviation", "#FFFFFF", "#8491B4"),
        Box(1070, 852, 285, 42, "Mixed oracle\n5×5 support; regret 0.203", "#FFFFFF", "#8491B4"),
    ]:
        dx.add_rect(box, 17, False)
    for a, b, color, dashed in [
        ((450, 278), (590, 278), "#00A087", False),
        ((590, 358), (450, 358), "#3C5488", False),
        ((1210, 278), (1350, 278), "#E64B35", False),
        ((1350, 358), (1210, 358), "#3C5488", False),
        ((420, 475), (1385, 480), "#3C5488", True),
        ((900, 480), (900, 570), "#8491B4", False),
        ((1495, 852), (1660, 452), "#8491B4", True),
    ]:
