...
\caption{Framework of the fixed-capacity inference-service simulation. Users choose between the API intermediary, direct provider access, and market exit. The intermediary routes traffic to two capacity-heterogeneous providers according to price and QoS. The simulation loop evaluates price shapes, user choice, routing, utilization, and the QoS fixed point, while the diagnostic layer reports finite-grid regret, QoS protection, and the profit boundary.}
...
During manuscript preparation, AI-assisted tools were used for language polishing, consistency checks, internal reviewer-style checks, and an author-review draft of Figure~\ref{fig:market_schematic}. The authors remain responsible for the simulation model, experimental artifacts, numerical results, interpretation, and final text. Because Elsevier's artwork policy does not permit generative AI to create or alter submitted figures unless such use is part of the research method, the current Figure~\ref{fig:market_schematic} artwork must be replaced by an author-approved non-AI artwork or cleared with the journal before formal submission.
