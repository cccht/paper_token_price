...
\title{\textbf{A Stylized Simulation Study of Time-of-Use Pricing for Fixed-Capacity Inference Services:\\
...
\begin{abstract}
Large language model (LLM) inference services operate under short-run fixed graphics processing unit (GPU) capacity, while time-varying arrivals can create off-peak idleness and peak quality-of-service (QoS) degradation. This paper develops a stylized, synthetic-calibrated simulation model to test whether time-of-use pricing can shift time-flexible inference demand away from congested periods and protect QoS without capacity expansion. The model combines two heterogeneous inference providers, one application programming interface (API) intermediary, time-rigid and time-flexible users, direct access, and an outside option. Provider prices are restricted to a low-dimensional time-shape family, and competition is evaluated as a finite candidate game with fictitious play and regret diagnostics. Verification uses fixed-point residuals and solver traces; validation is bounded to public price scales and controlled vLLM overload measurements. In the congested uniform-pricing baseline, peak utilization is $0.782$ and minimum QoS is $0.756$. Dynamic coarse-grid and fine-grid snapshots reduce peak utilization to $0.706$ and $0.666$, respectively, and raise minimum QoS to $0.968$ and $0.990$. The principal finite-grid mixed diagnostic on the full 225-point grid gives a $5\times5$ support profile with maximum regret $0.203$, expected peak utilization $0.703$, and minimum QoS $0.970$. Additional service-management baselines and stress checks keep the QoS direction positive, whereas profit is solver-dependent: $+9.3\%$ for the coarse snapshot, $-1.9\%$ for the fine snapshot, and about $-2.8\%$ for the low-regret mixed profile. Evidence supports time-of-use pricing as a bounded QoS-protection mechanism, not as a production forecast or reliable profit-improvement result.
\end{abstract}
...
\section{Related Research Review}
\label{sec:related}

\paragraph{Inference serving and QoS-aware market simulation.}
...
\paragraph{Electricity dynamic pricing as a structural template.}
...
\paragraph{Congestion pricing, revenue management, and platform choice.}
...
\paragraph{Market structure and sequence of decisions.}
...
\paragraph{User choice and the outside option.}
...
\paragraph{Profit with fixed-capacity holding cost.}
...
\paragraph{Solution method and simulation diagnostics.}
\label{sec:solver}

\paragraph{Low-dimensional pricing-shape family.}
...
\paragraph{Candidate evaluation and finite-game equilibrium certificate.}
...
\paragraph{Fictitious play and finite-grid regret.}
...
\paragraph{Verification, validation, and credibility boundaries.}
\label{sec:vv}
...
\paragraph{Service-management baselines and structural checks.}
...
\paragraph{Local parameter stress tests.}
...
\paragraph{Surplus reallocation rather than additional supply.}
...
\paragraph{Why recovered QoS losses do not reliably become profit.}
...
\paragraph{Operational and regulatory implications.}
...
\paragraph{Reproducibility, data availability, and artifact boundary.}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \path{experiments/run_peak_shaving_smpt_experiments.py}: service-management baselines, ablations, phase grid, restricted local re-solve, and fixed-point residual diagnostics;
...
The editable source for Figure~\ref{fig:market_schematic} is \path{figure_sources/peak_shaving_framework_2026-07-11.drawio}, and its deterministic builder is \path{figure_sources/build_peak_shaving_framework_drawio.py}. The manuscript uses the PDF 1.3 compatibility export; lossless PNG and SVG exports are retained with the source. The locally pinned Streamline icon files, source mapping, attribution text, and CC BY 4.0 licence are stored under \path{figure_sources/iot_icon_candidates/streamline_ultimate_color/}.
...
During manuscript preparation, AI-assisted tools were used for language polishing, consistency checks, internal reviewer-style checks, and layout and consistency checks for the editable vector diagram. No generative image model was used to create or alter the vector artwork in Figure~\ref{fig:market_schematic}. The figure was constructed from the model equations and simulation workflow using licensed Streamline icons and was reviewed by the authors. The authors remain responsible for the simulation model, experimental artifacts, numerical results, interpretation, text, and figures.
