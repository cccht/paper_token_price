...
\caption{Profit distribution and single-snapshot convergence boundary. The coarse grid reaches regret below 5, whereas the fine-grid pure snapshot remains at regret $22.3$ after 40 rounds. The fine-grid pure snapshot is therefore used only as a resolution stress test, not as a robust equilibrium conclusion for profit or strategy intensity.}
...
\caption{Mechanism diagnostics. Dynamic pricing raises QoS-adjusted served volume and lowers exit probabilities, but the average paid price falls at the same time. The improvement is closer to better coverage and user experience than to a stable profit increase.}
\label{fig:mechanism}
\end{figure}
...
A public project repository has been created at \url{https://github.com/cccht/paper_token_price}. Because the present submission draft may include local revisions made after the latest repository push, a formal submission should cite a frozen release or archival digital object identifier that contains the final manuscript, code, configuration files, raw JSON/CSV artifacts, figure scripts, and generated figures. Until that archival release is created, the GitHub repository should be treated as a living reproducibility package rather than a persistent data identifier.
